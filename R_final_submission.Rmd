...
library(dplyr)
library(pheatmap)
# Load the hexbin package (needed for hexagonal binning)
library(hexbin) 
```
...
df %>%
  ...
  summarise(
    ...
    n_mechvent_yes = sum(mech_vent == "Yes", na.rm=TRUE), # Count patients with 
                                                          # mechanical ventilation
    n_mechvent_no  = sum(mech_vent == "No", na.rm=TRUE),  # Count patients without
                                                          # mechanical ventilation
    .groups="drop"
  )
...
ggplot(df2d, aes(age_num, A1BG_value)) +   
  
  # Use hexagonal binning instead of scatterplot (reduces overplotting for large datasets)
  geom_hex(bins = 25) +                  
   
  # Split the plot by ICU status into separate panels (faceting)
  facet_wrap(~ icu_status, nrow = 1) +   
  
  # Keep axes tight to data, no extra padding around
  coord_cartesian(expand = FALSE) +      
  
  # Use Viridis color scale (better perceptual uniformity)
  scale_fill_viridis_c() +               
  
  # Add descriptive labels and titles
  labs(     
    title = "Hexbin of Age vs A1BG by ICU Status",  # Plot title
    x = "Age (years)",                              # X-axis label
    y = "A1BG Expression",                          # Y-axis label
    fill = "Count"                                  # Legend label (number of 
                                                    # samples in each hexagon)
  ) +   
  
  # Apply a clean theme with base font size 14
  theme_minimal(base_size = 14) +   
  
  # Customize title, axis, and facet label styles
  theme(     
